...
for n1 = 1:length(presVals)
    ...
    data.(fieldName).p = tmpData(:,7)*6894.75;          % Pa. Pressure upstream of article.
...
summary = zeros(length(presVals),6);
...
%Clean up Data
    ...
    summary(n1,:) = [data.(fieldName).p_avg/1000,data.(fieldName).mDot,data.(fieldName).Cd,data.(fieldName).CDR1,data.(fieldName).CDR2,data.(fieldName).D.CdRel];
