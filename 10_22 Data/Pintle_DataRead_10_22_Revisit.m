...
for n1 = 1:length(presVals)
    ...
    data.(fieldName).p = tmpData(:,7)*6894.75; % Pressure upstream of article in psi
    data.(fieldName).lc_1 = tmpData(:,16)*(1/2.205); % Load Cell (lbf)
    data.(fieldName).lc_2 = tmpData(:,17)*(1/2.205);
    data.(fieldName).lc_3 = tmpData(:,18)*(1/2.205);
    data.(fieldName).lc_4 = tmpData(:,19)*(1/2.205);
...
for n1 = 1:length(presVals)
    for n2 = 1:nRuns(n1)
        ...
        [data.(fieldName).flowLineR,data.(fieldName).SR]= polyfit(data.(fieldName).t(minInd:maxInd),data.(fieldName).m(minInd:maxInd),1);
        ...
        data.(fieldName).CDR1 = data.(fieldName).flowLineR(1)/(A*sqrt(2*rho*data.(fieldName).p_avg));
        data.(fieldName).CDR2 = mean(data.(fieldName).flowLineR(1)./(A*sqrt(2*rho*data.(fieldName).p(minInd:maxInd))));
        ...
        summary(currRun,:) = [data.(fieldName).p_avg/1000,data.(fieldName).mDot,data.(fieldName).Cd,data.(fieldName).CDR1,data.(fieldName).CDR2,data.(fieldName).D.CdRel];
