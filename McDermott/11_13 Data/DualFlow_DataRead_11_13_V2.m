...
filenamePrefix = "Data\";
filenameExtension = "_21-11-13.csv";
presVals = [163,29;...
            214,73;...
            217,72];
nRuns = [2,1,3];
...
for n1 = 1:size(presVals,1)
    for n2 = 1:nRuns(n1)
        filename = filenamePrefix +"A"+string(presVals(n1,1))+"P"+string(presVals(n1,2))+"Test"+string(n2)+filenameExtension;
    ...
    if isempty(dualFlow)
        ...
        if data.(fieldName).mTotMeas < 0
            try
                ...
                data.(fieldName).mTotMeas = data.(fieldName).m(data.(fieldName).res(n2).maxInd+expandRange/2) - data.(fieldName).m(1);
            end
        end
