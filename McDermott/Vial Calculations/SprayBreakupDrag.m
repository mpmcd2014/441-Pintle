function [rf,tf,x,v,t] = SprayBreakupDrag(droplet, medium, h0, L, v0, theta0, dt)
...
rb  = breakupRadius(droplet.rho,medium.rho,v0,L,droplet.sigma);
...
while (x(n1,2) > 0) && (n1 <= iterLim)
    if rssq(x(n1,1:2)) < rb
        x(n1+1,1) = v0*sind(theta0)*dt;
        x(n1+1,2) = -v0*cosd(theta0)*dt;
    else
        ...
        if abs(v(n1+1,3)-v(n1,3)) < 1e-2 % If the droplet is terminal
            theta(n1+1)     = 0;
            x(n1+1,:)       = [x(n1,1),0];
            t(n1+1)         = x(n1,2)/v(n1+1,3);
            n1 = n1+1;
            continue
        else
