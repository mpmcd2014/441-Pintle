...
printResults = {'TMRAvg','ThetaAvg','mTotCalc','mTotMeas'};
%% Read in Data
%%some intializations
...
for n1 = 1:size(presVals,1)
    for n2 = 1:nRuns(n1)
        ...
        %data.(fieldName).ann    = struct();
        %data.(fieldName).pin    = struct();
        %data.(fieldName).ann.P  = tmpData(:,6)*6894.75;          % Pa. Pressure upstream of annulus.
        %data.(fieldName).pin.P  = tmpData(:,7)*6894.75;       % Pa. Pressure upstream of pintle.
...
%Clean up Data
    ...
    %{
    tmp = FilterData_V1(data.(fieldName).t,[data.(fieldName).ann.P,data.(fieldName).pin.P,data.(fieldName).m],freqThreshold);
    data.(fieldName).ann.Psmooth = tmp(:,1);
    data.(fieldName).pin.Psmooth = tmp(:,2);
    data.(fieldName).msmooth = tmp(:,3);
    
    data.(fieldName).ann.minInd = find((data.(fieldName).Psmooth-Pvary*max(data.(fieldName).Psmooth))>0,1,'first');
    data.(fieldName).ann.maxInd = find((data.(fieldName).Psmooth-Pvary*max(data.(fieldName).Psmooth))>0,1,'last');
    %}
    for n2 = 1:2
        ...
        data.(fieldName).res(n2).Pavg = mean(data.(fieldName).res(n2).Psmooth(data.(fieldName).res(n2).minInd:data.(fieldName).res(n2).maxInd));
        data.(fieldName).res(n2).mDotAvg = mean(data.(fieldName).res(n2).mDot(data.(fieldName).res(n2).minInd:data.(fieldName).res(n2).maxInd));
    end
    ...
    data.(fieldName).TMRAvg     = mean(data.(fieldName).TMR);
    data.(fieldName).ThetaAvg = mean(data.(fieldName).Theta);
