...
%% DROPLET WITH DRAG
diameters   = [10e-6,25e-6,50e-6,100e-6,250e-6,500e-6];
...
droplet = struct('rho',998);
for n1 = 1:length(diameters)
    droplet.D   = diameters(n1);
    [drag(n1).rf,tf,x,drag(n1).v,drag(n1).t] = SprayDragPrediction(droplet, 'air', H, v0, thetaR, 1e-4);
    drag(n1).x  = x(:,1);
    drag(n1).y  = x(:,2);
