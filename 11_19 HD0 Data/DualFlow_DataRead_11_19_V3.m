...
filenamePrefix = "Data\";
filenameExtension = "_21-11-19_HD0";
...
mTestComp = zeros(length(testNames),4);
...
%Clean up Data
    ...
    mTestComp(n1,3) = (1/10)*(mean(data.(fieldName).m((dualFlow(end)-10):(dualFlow(end)+10)))-mean(data.(fieldName).m((dualFlow(1)-10):(dualFlow(1)+10))));
    
    [data.(fieldName).flowLine,data.(fieldName).S]= polyfit(data.(fieldName).t(dualFlow),data.(fieldName).msmooth(dualFlow),1);
    %Plots
    %{
    figure
    plot(data.(fieldName).t,data.(fieldName).m)
    hold on
    plot(data.(fieldName).t(dualFlow),polyval(data.(fieldName).flowLine,data.(fieldName).t(dualFlow)))
    data.(fieldName).ax = gca();
    data.(fieldName).ax.FontName = 'Times New Roman';
    data.(fieldName).ax.FontSize = 18;
    %}
    mTestComp(n1,4) = (1/10)*data.(fieldName).flowLine(1)*(data.(fieldName).t(dualFlow(end)) - data.(fieldName).t(dualFlow(1)));
